...
\begin{enumerate}
  \item \textbf{[Prepares for CS336 A1]} Write a function `rope(x)` that takes a tensor `x` of shape `(B, T, d)` and correctly applies broadcasting to add a learned positional encoding `bias` of shape `(T, d)` to it without using loops or `.expand()`.
\end{enumerate}
