...
\begin{equation}
\vx_{\text{new}} = \vx + \text{Sublayer}(\vx)
\end{equation}

Mathematically, when you take the derivative of the loss with respect to $\vx$, you get:
\begin{equation}
\frac{\partial \mathcal{L}}{\partial \vx} = \frac{\partial \mathcal{L}}{\partial \vx_{\text{new}}} \left( \vI + \frac{\partial \text{Sublayer}(\vx)}{\partial \vx} \right)
\end{equation}
...
\begin{enumerate}
  \item \textbf{[Prepares for CS336 A1]} Implement a PyTorch \texttt{nn.Module} named \texttt{TransformerBlock}. It should take \texttt{x} (shape $B, T, d_m$), pass it through your simulated MHA and FFN modules using standard PyTorch \texttt{nn.LayerNorm}, and output the residual-summed tensor. Use no biases in the linear layers.
\end{enumerate}
